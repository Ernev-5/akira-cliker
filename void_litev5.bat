...
set "KEY_TOGGLE=Oem5"
:: Il tasto per disabilitare il clicker e tornare al menu
set "KEY_DISABLE=Delete"
:: Il tasto per nascondere/mostrare la finestra
set "KEY_HIDE=Home"
...
namespace n$namespace
{
	public class c$class
	{
		...
		private static bool IsClickAllowed()
		{
			// Tasto sinistro del mouse (VK_LBUTTON = 1)
			bool leftClickPressed = BitConverter.GetBytes(GetAsyncKeyState(1))[1] == 0x80;
			// Tasto Shift (VK_SHIFT = 16)
			bool shiftPressed = BitConverter.GetBytes(GetAsyncKeyState(16))[1] == 0x80;
			
			return leftClickPressed && !shiftPressed;
		}
					...
					if (IsClickAllowed())
					{
						if (MCWindow == ForegroundWindow)
						{
							if (lastLoopRun == 0) {
								...
								else if (cpsDrop > 0) minDelay += GetRandomDouble(1, 15);

								double maxDelay = 1000 / MaximumCPS;
								if (cpsSpike > 0) maxDelay -= GetRandomDouble(1, 15);
								else if (cpsDrop > 0) maxDelay += GetRandomDouble(1, 15);

								double delay = (Math.Sin(sinX) + 1) * (maxDelay - minDelay) / 2 + minDelay;
								sinX += 0.1;

								int delayInt = (int) Math.Round(delay);
								if (delayInt < 1) delayInt = 1;

								SendMessage(ForegroundWindow, 0x0201, (UIntPtr) 0x0001, MAKELPARAM(Cursor.Position.X, Cursor.Position.Y));
								Thread.Sleep(delayInt >> 1);
								SendMessage(ForegroundWindow, 0x0202, UIntPtr.Zero, MAKELPARAM(Cursor.Position.X, Cursor.Position.Y));
								Thread.Sleep(delayInt >> 1);
...
{
	foreach ($err in $CompilerResults.Errors)
	{
		echo $err.ErrorText
	}
}
else
{
	...
	$instance = $assembly.CreateInstance("n$namespace.c$class")
